...
- не усложнять первую реализацию лишней инфраструктурой и фреймворками.
...
- `Custom-core`: собственный runtime и retrieval engine без внешних agent/RAG framework core;
...
- `Provider abstraction by capability`: замена модельного провайдера должна делаться конфигом.
...
Первая версия должна использовать `Gemini` только через `REST API + API key`, без SDK.
...
- `Agent Runtime`: `ControllerAgent`, `Composer`, `GroundingVerifier`;
...
В системе должен быть один основной агент `ControllerAgent`.
...
Архитектура должна использовать capability-based интерфейсы:
...
- интеграция Gemini инкапсулируется в отдельном provider adapter;
- ядро не зависит от SDK или vendor-specific runtime;
- провайдер меняется через конфиг;
- позднее должна быть возможна замена на другой HTTP-compatible adapter или локальную модель.
...
- допускает замену модельного провайдера через provider abstraction.
...
- использование внешних agent/RAG frameworks как ядра системы.
